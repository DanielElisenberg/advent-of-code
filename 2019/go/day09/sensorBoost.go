...
func main() {
	...
	fmt.Println("Starting up intcode computer")
	var in int64
	var i int64 = 0
processLoop:
	for true {
		opcode, firstMode, secondMode, thirdMode := parseInstruction(memory[i])
		switch opcode {
		case 1:
			memory[getValue(thirdMode, i+3, memory)] = memory[getValue(secondMode, i+2, memory)] + memory[getValue(firstMode, i+1, memory)]
			i += 4
		case 2:
			memory[getValue(thirdMode, i+3, memory)] = memory[getValue(secondMode, i+2, memory)] * memory[getValue(firstMode, i+1, memory)]
			i += 4
		...
		case 7:
			if memory[getValue(firstMode, i+1, memory)] < memory[getValue(secondMode, i+2, memory)] {
				memory[getValue(thirdMode, i+3, memory)] = 1
			} else {
				memory[getValue(thirdMode, i+3, memory)] = 0
			}
			i += 4
		case 8:
			if memory[getValue(firstMode, i+1, memory)] == memory[getValue(secondMode, i+2, memory)] {
			...
			i += 4
		case 9:
			relativeBase += memory[getValue(firstMode, i+1, memory)]
			i += 2
		case 99:
			break processLoop
		default:
...
func getValue(mode int64, parameter int64, memory map[int64]int64) int64 {
	switch mode {
	case 0:
...
func parseInstruction(instruction int64) (int64, int64, int64, int64) {
	var thirdMode int64 = instruction / 10000
	instruction -= 10000 * thirdMode
	var secondMode int64 = instruction / 1000
	instruction -= 1000 * secondMode
	var firstMode int64 = instruction / 100
	instruction -= 100 * firstMode
	var opcode int64 = instruction
	return opcode, firstMode, secondMode, thirdMode
}

func arrayFrom(data string) map[int64]int64 {
